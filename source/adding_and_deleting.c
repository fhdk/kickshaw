...
void one_of_the_change_values_buttons_pressed (gchar *origin)
{
    ...
    if (STREQ (origin, "reset")) {
        ...
        else { // In case that both the label and command field were not filled out.
            gtk_style_context_remove_class (gtk_widget_get_style_context (ks.entry_fields[MENU_ELEMENT_OR_VALUE_ENTRY]), 
                                            "bg_class");
        }
        ...
        if (STREQ (new_menu_element, "item") && including_action_check_button_active &&
            ...
            gtk_style_context_remove_class (gtk_widget_get_style_context (ks.options_fields[COMMAND]), "bg_class");
        }
...
void add_new (gchar *new_menu_element_plus_opt_suppl)
{
    ...
    */
    ...
    if (!streq_any (new_menu_element, "menu", "pipe menu", "item", NULL)) {
        ...
        gtk_widget_set_visible (ks.entry_labels[ICON_PATH_ENTRY], !STREQ (new_menu_element, "separator"));
        gtk_widget_set_visible (ks.entry_fields[ICON_PATH_ENTRY], !STREQ (new_menu_element, "separator"));
...
void show_action_options (void)
{
    ...
    if (STREQ (combo_choice, "Execute")) {
        for (options_cnt = 0; options_cnt < NUMBER_OF_EXECUTE_OPTS; options_cnt++) {
            ...
            gtk_label_set_text (GTK_LABEL (ks.options_labels[options_cnt]), ks.options_label_txts[options_cnt]);
            gtk_widget_show (ks.options_labels[options_cnt]);
            gtk_widget_show (ks.options_fields[options_cnt]);
        }
        if (!g_signal_handler_is_connected (ks.options_fields[SN_OR_PROMPT], ks.handler_id_show_startupnotify_options)) {
    ...
    if (streq_any (combo_choice, "Exit", "Prompt", "SessionLogout", NULL)) {
         if (!STREQ (combo_choice, "Prompt") || 
            ...
            if (g_signal_handler_is_connected (ks.options_fields[SN_OR_PROMPT], ks.handler_id_show_startupnotify_options)) {
                g_signal_handler_disconnect (ks.options_fields[SN_OR_PROMPT], ks.handler_id_show_startupnotify_options);
            }
            gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (ks.options_fields[SN_OR_PROMPT]), TRUE);
...
void show_startupnotify_options (void)
{
  gtk_widget_set_visible (ks.suboptions_grid, 
                          gtk_toggle_button_get_active (GTK_TOGGLE_BUTTON (ks.options_fields[SN_OR_PROMPT])));
}
...
static void clear_entries (void)
{
    ...
    for (options_cnt = PROMPT; options_cnt <= COMMAND; options_cnt++) {
        ...
        gtk_style_context_remove_class (gtk_widget_get_style_context (ks.options_fields[options_cnt]), "bg_class");
    }
    ...
    for (snotify_opts_cnt = NAME; snotify_opts_cnt < NUMBER_OF_STARTUPNOTIFY_OPTS; snotify_opts_cnt++) {
        ...
        gtk_style_context_remove_class (gtk_widget_get_style_context (ks.suboptions_fields[snotify_opts_cnt]), "bg_class");
    }
}
