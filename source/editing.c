                   ...
                   gboolean     display_err_msg)
{
    ...
    if (G_UNLIKELY (!(icon_in_original_size = gdk_pixbuf_new_from_file (icon_path, &error)))) {
        if (display_err_msg) {
            ...
            g_string_append_printf (error_message, ":\n\n<span foreground='darkred'>%s</span>", error->message);
        ...
        if (!STREQ (ks.txt_fields[TYPE_TXT], "separator")) {
            ...
            if (streq_any (ks.txt_fields[TYPE_TXT], "menu", "pipe menu", NULL)) {
                if (STREQ (ks.txt_fields[TYPE_TXT], "pipe menu") && !*entry_txt[EXECUTE_ENTRY]) {
                    show_errmsg ("Pipe menus need a command to execute.");
                    set_entry_fields ();
                    return;
                }
            ...
            if (ks.txt_fields[ICON_PATH_TXT] && !(*entry_txt[ICON_PATH_ENTRY])) {
                ...
                if (set_icon (&ks.iter, (gchar *) entry_txt[ICON_PATH_ENTRY], TRUE)) {
                                ...
                                gpointer             column_number_pointer)
{
    ...
    GtkTreeSelection *selection = gtk_tree_view_get_selection (GTK_TREE_VIEW (ks.treeview));
    guint number_of_selected_rows = gtk_tree_selection_count_selected_rows (selection);
    ...
    if (column_number == COL_EXECUTE && !(*new_text)) {
        show_errmsg ("Pipe menus need a command to execute.");
        return;
    }
...
static void empty_label_msg (void)
{
    ...
    create_dialog (&dialog, "Entry of empty label blocked", "dialog-error", label_txt, "_Close", NULL, NULL, TRUE);
